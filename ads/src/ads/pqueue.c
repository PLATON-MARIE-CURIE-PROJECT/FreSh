...
// ------------------------------------
// EKOSMAS - LOCK FREE VERSIONS - START

static int
percolate_down_lf(pqueue_t *q, size_t moving_node_pos, MESSI_workerdata_ekosmas_lf *input_data)
{
    size_t child_node;
    size_t i = 1;
    void *moving_node = q->d[moving_node_pos];
    pqueue_pri_t moving_pri = q->getpri(moving_node);

    while ((child_node = maxchild(q, i)) &&
           q->cmppri(moving_pri, q->getpri(q->d[child_node])))
    {
        if (input_data->helper_queue_exist[input_data->workernumber])
            return 0;
        q->d[i] = q->d[child_node];
        q->setpos(q->d[i], i);
        i = child_node;
...
void *
pqueue_pop_lf(pqueue_t *q, MESSI_workerdata_ekosmas_lf *input_data)
{
    void *head;
    ...
    head = q->d[1];
    // q->d[1] = q->d[--q->size];
    if (percolate_down_lf(q, cur_size-1, input_data)) 
        q->size = cur_size - 1;

    return head;
}

void *pqueue_top_lf(pqueue_t *q)
{
    if (!q || q->size == 1)
        return NULL;
...
// EKOSMAS - LOCK FREE VERSIONS - END
// ----------------------------------
...
void heapify(pqueue_t *q, int n, int i) {
    // Find smallest among root, left child and right child
    int smallest = i;
    // int left = 2 * i + 1;
    // int right = 2 * i + 2;
    // pqueue_pri_t moving_pri = q->getpri(q->d[i]);

    if (left(i) < n && q->cmppri(q->getpri(q->d[left(i)]), q->getpri(q->d[smallest])))
        smallest = left(i);

    if (right(i) < n && q->cmppri(q->getpri(q->d[right(i)]), q->getpri(q->d[smallest])))
        smallest = right(i);

    // Swap and continue heapifying if root is not smallest
    if (smallest != i) {
        swap(&q->d[i], &q->d[smallest]);
...
void heap_sort(pqueue_t *q, int n) {
    // // Build min heap
    // for (int i = n / 2; i > 0; i--)
    //     heapify(q, n+1, i);
